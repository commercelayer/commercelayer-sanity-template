...
callback();}const emacsLockfilePattern='**/.#*';let webpackConfig={externals:!isServer?// make sure importing "next" is handled gracefully for client
...
entry:async()=>{return{...(clientEntries?clientEntries:{}),...entrypoints,...(isServer?{'init-server.js':'next-plugin-loader?middleware=on-init-server!','on-error-server.js':'next-plugin-loader?middleware=on-error-server!'}:{})};},watchOptions:{ignored:['**/.git/**','**/node_modules/**','**/.next/**',// can be removed after https://github.com/paulmillr/chokidar/issues/955 is released
emacsLockfilePattern]},output:{...(isWebpack5?{environment:{arrowFunction:false,bigIntLiteral:false,const:false,destructuring:false,dynamicImport:false,forOf:false,module:false}}:{}),path:outputPath,// On the server we don't use the chunkhash
...
alias:['emit-file-loader','error-loader','next-babel-loader','next-client-pages-loader','next-serverless-loader','noop-loader','next-plugin-loader'].reduce((alias,loader)=>{// using multiple aliases to replace `resolveLoader.modules`
...
if(resolvedBaseUrl){var _webpackConfig$resolv,_webpackConfig$resolv2;(_webpackConfig$resolv=webpackConfig.resolve)==null?void 0:(_webpackConfig$resolv2=_webpackConfig$resolv.modules)==null?void 0:_webpackConfig$resolv2.push(resolvedBaseUrl);}if((_jsConfig2=jsConfig)!=null&&(_jsConfig2$compilerOp=_jsConfig2.compilerOptions)!=null&&_jsConfig2$compilerOp.paths&&resolvedBaseUrl){var _webpackConfig$resolv3,_webpackConfig$resolv4;(_webpackConfig$resolv3=webpackConfig.resolve)==null?void 0:(_webpackConfig$resolv4=_webpackConfig$resolv3.plugins)==null?void 0:_webpackConfig$resolv4.unshift(new _jsconfigPathsPlugin.JsConfigPathsPlugin(jsConfig.compilerOptions.paths,resolvedBaseUrl));}if(isWebpack5){var _webpackConfig$output;// futureEmitAssets is on by default in webpack 5
...
version:`${"10.0.5"}|${nextPublicVariables}|${nextEnvVariables}|${configVars}`,cacheDirectory:_path.default.join(dir,'.next','cache','webpack')};// Adds `next.config.js` as a buildDependency when custom webpack config is provided
