...
function generateCommandsDocumentation(commandGroups, group = 'default') {
  ...
  const rows = [`usage: sanity${prefix} [--default] [-v|--version] [-d|--debug] [-h|--help] <command> [<args>]`, '', 'Commands:'].concat(commands.map(cmd => `   ${(0, _lodash.padEnd)(cmd.name, cmdLength + 1)} ${cmd.description}`)).concat(['', `See 'sanity help${prefix} <command>' for specific information on a subcommand.`]);
  return rows.join('\n');
}
/**
