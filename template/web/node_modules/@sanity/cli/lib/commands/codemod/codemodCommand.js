...
var _default = {
  ...
  async action(args, context) {
    ...
    const cmdArgs = ['jscodeshift', '--ignore-pattern', 'node_modules', '--ignore-pattern', 'dist', hasGitIgnore && '--ignore-config', hasGitIgnore && '.gitignore', '-t', modPath, '--extensions', exts.join(','), dryRun && '--dry', workDir].filter(Boolean);
...
function ensureNpx() {
  try {
    const npxHelp = childProcess.execSync('npx --help', {
      encoding: 'utf8'
    });
