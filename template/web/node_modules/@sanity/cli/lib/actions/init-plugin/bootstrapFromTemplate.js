...
module.exports = async (context, url) => {
  ...
  const manifest = zip.find(file => path.basename(file.path) === 'package.json' && !file.path.includes('node_modules'));
  ...
  if (minimumCliVersion) {
    ...
    if (semver.lt(pkg.version, minimumCliVersion)) {
      throw new Error(`Template requires @sanity/cli at version ${minimumCliVersion}, installed is ${pkg.version}`);
    }
  }
  ...
  const name = await prompt.single({
    ...
    validate: async pkgName => {
      const {
        validForNewPackages
      } = validateNpmPackageName(pkgName);
