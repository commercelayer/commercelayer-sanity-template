...
describe("Accesspoint ARN", async () => {
    const endpointValidator = (next, context) => (args) => {
        // middleware intercept the request and return it early
        const request = args.request;
    ...
    it("should succeed with access point ARN", async () => {
        const client = new S3_1.S3({ region: "us-west-2" });
        ...
        chai_1.expect(result.request.hostname).to.eql("myendpoint-123456789012.s3-accesspoint.us-west-2.amazonaws.com");
    });
    it("should sign request with region from ARN is useArnRegion is set", async () => {
        const client = new S3_1.S3({
            region: "us-east-1",
            useArnRegion: true,
        ...
        chai_1.expect(result.request.headers.authorization).to.contain("/us-west-2/s3/aws4_request, SignedHeaders=");
    });
    it("should succeed with outposts ARN", async () => {
        ...
        const client = new S3_1.S3({ region: "us-east-1", credentials, useArnRegion: true });
        client.middlewareStack.add(endpointValidator, { step: "finalizeRequest", priority: "low" });
        const result = await client.putObject({
        ...
        chai_1.expect(result.request.hostname).to.eql(`abc-111-${AccountId}.${OutpostId}.s3-outposts.us-west-2.amazonaws.com`);
        chai_1.expect(result.request.headers["authorization"]).contains(`Credential=${credentials.accessKeyId}/20200928/${region}/s3-outposts/aws4_request`);
    });
});
